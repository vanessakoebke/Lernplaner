...
public class AufgabenManager {
    ...
    public AufgabenManager(Control control) {
        this.control = control;
        aufgabenListe = new ArrayList<Aufgabe>();
    }
    
    public AufgabenManager(List<Aufgabe> aufgabenListe, Control control) {
        this.control = control;
    ...
    public List<Aufgabe> getAufgabenListe(Modul modul) {
        ...
        if (modul != null) {
            for (Aufgabe a : aufgabenListe) {
                if (a.getModul() != null && a.getModul().getId().equals(modul.getId())) {
                    result.add(a);
                }
            } 
        }
...
//    }
    ...
    public void statusAendern(Aufgabe aufgabe, Status neuerStatus) {
        aufgabe.setStatus(neuerStatus);
        control.getDb().upsertAufgabe(aufgabe); // Speichern in DB

        // Prüfen, ob Folgeaufgabe existiert
        Aufgabe folge = aufgabe.getFolgeAufgabe();
        if (folge != null && neuerStatus == Status.ERLEDIGT) {
            folge.setStart(LocalDate.now());
            control.getDb().upsertAufgabe(folge); // DB aktualisieren
        }
    }
}
